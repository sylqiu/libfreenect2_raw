...
public:
  ...
  void processMeasurementTriple(float trig_table[512*424][6], float abMultiplierPerFrq, int x, int y, const int32_t* m, float* m_out)
  {
    ...
    if (0 < zmultiplier)
    {
      bool saturated = (m[0] == 32767 || m[1] == 32767 || m[2] == 32767); //2^15=32768
      if (!saturated)
      {
        int offset = y * 512 + x;
  ...
  void processPixelStage1(int x, int y, unsigned char* data, float *m0_out, float *m1_out, float *m2_out,\
                          float *m0_raw0, float *m0_raw1, float *m0_raw2,\
                          float *m1_raw0, float *m1_raw1, float *m1_raw2,\
                          float *m2_raw0, float *m2_raw1, float *m2_raw2)
  {
  ...
  void filterPixelStage1(int x, int y, const Mat<Vec<float, 9> >& m, float* m_out, bool& bilateral_max_edge_test)
  {
    ...
    else
    {
      ...
      {
        float norm2 = m_ptr[0] * m_ptr[0] + m_ptr[1] * m_ptr[1];  // a^2 + b^2
        float inv_norm = 1.0f / std::sqrt(norm2);  // 1 / (a^2 + b^2)^(-1)
  ...
  void processPixelStage2(int x, int y, float *m0, float *m1, float *m2, float *ir_out, float *depth_out, float *ir_sum_out)
  {
    ...
    transformMeasurements(m0); //[a,b,amp]
...
void CpuDepthPacketProcessor::process(const DepthPacket &packet)
{
  ...
  // float m0_raw0 [424][512];
  // float m0_raw1 [424][512];
  // float m0_raw2 [424][512];
  // float m1_raw0 [424][512];
  // float m1_raw1 [424][512];
  // float m1_raw2 [424][512];  
  // float m2_raw0 [424][512];
  // float m2_raw1 [424][512];
  // float m2_raw2 [424][512];
 
  // float raw_measurement [424][512][9];
  float* raw_measurement;
  raw_measurement = (float *) malloc(1953792 * sizeof(float));
  for(int y = 0; y < 424; ++y)
    for(int x = 0; x < 512; ++x, m_ptr += 9)
    {
      ...
      impl_->processPixelStage1(x, y, packet.buffer, m_ptr + 0, m_ptr + 3, m_ptr + 6, &a0, &a1, &a2, &b0, &b1, &b2, &c0, &c1, &c2);
      int ytmp = 423 - y;
      // raw_measurement[423-y][x][0] = a0;
      // raw_measurement[423-y][x][1] = a1;
      // raw_measurement[423-y][x][2] = a2;
      // raw_measurement[423-y][x][3] = b0;
      // raw_measurement[423-y][x][4] = b1;
      // raw_measurement[423-y][x][5] = b2;
      // raw_measurement[423-y][x][6] = c0;
      // raw_measurement[423-y][x][7] = c1;
      // raw_measurement[423-y][x][8] = c2;
      // if(x == 0 & y ==0){
      //   // printf("!!!!!! %f", a0);
      //   raw_measurement[0] = a0;
      //   printf("!!!!!! %f", raw_measurement[0]);
      
      // }
      
      // raw_measurement[0*424*512+x*424+ytmp] = a0;
      // raw_measurement[1*424*512+x*424+ytmp] = a1;
      // raw_measurement[2*424*512+x*424+ytmp] = a2;
      // raw_measurement[3*424*512+x*424+ytmp] = b0;
      // raw_measurement[4*424*512+x*424+ytmp] = b1;
      // raw_measurement[5*424*512+x*424+ytmp] = b2;
      // raw_measurement[6*424*512+x*424+ytmp] = c0;
      // raw_measurement[7*424*512+x*424+ytmp] = c1;
      // raw_measurement[8*424*512+x*424+ytmp] = c2;

      raw_measurement[ytmp*512*9+x*9+0] = a0;
      raw_measurement[ytmp*512*9+x*9+1] = a1;
      raw_measurement[ytmp*512*9+x*9+2] = a2;
      raw_measurement[ytmp*512*9+x*9+3] = b0;
      raw_measurement[ytmp*512*9+x*9+4] = b1;
      raw_measurement[ytmp*512*9+x*9+5] = b2;
      raw_measurement[ytmp*512*9+x*9+6] = c0;
      raw_measurement[ytmp*512*9+x*9+7] = c1;
      raw_measurement[ytmp*512*9+x*9+8] = c2;

      // printf("%f\t" , m0_raw0[y][x]);
      // printf("%f\t" , m0_raw1[y][x]);
      // printf("%f\n" , m0_raw2[y][x]);
    }  
    

  // save raw_measurements
  

  // uint32_t pic_num =packet.timestamp;
  // printf("%d", pic_num);
  // FILE* outfile =NULL;

  // char file_name1[100] = "../recordings/raw_measurements/";
  // char file_name2[20];
  // sprintf(file_name2, "%d", pic_num);
  // char file_name3[] = ".bin";
  // strcat(file_name1, file_name2);
  // strcat(file_name1, file_name3);
  // // printf("!!!!!!!!!!!!");
  // outfile = fopen(file_name1 , "wb");
  // // printf("%f", raw_measurement[0]);
  // // printf("%d", sizeof(float));
  // // float tmp[2][2][2];
  // fwrite(raw_measurement, sizeof(float), 1953792, outfile);
  // fclose(outfile);
  // free(raw_measurement);
  ...
  if(impl_->enable_bilateral_filter)
  {
    ...
    for(int y = 0; y < 424; ++y)
      for(int x = 0; x < 512; ++x, m_filtered_ptr += 9, ++m_max_edge_test_ptr)
      {
        ...
        impl_->filterPixelStage1(x, y, m, m_filtered_ptr, max_edge_test_val);  // m & m_filtered_ptr shape:(424,512)
