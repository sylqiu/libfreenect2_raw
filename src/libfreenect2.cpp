...
void Freenect2DeviceImpl::setColorCameraParams(const Freenect2Device::ColorCameraParams &params)
{
  ...
  float cx = params.cx;
  float cy = params.cy;
  float shift_d = params.shift_d;
  float shift_m = params.shift_m;
  float mx_x3y0 = params.mx_x3y0; // xxx
  float mx_x0y3 = params.mx_x0y3; // yyy
  float mx_x2y1 = params.mx_x2y1; // xxy
  float mx_x1y2 = params.mx_x1y2; // yyx
  float mx_x2y0 = params.mx_x2y0; // xx
  float mx_x0y2 = params.mx_x0y2; // yy
  float mx_x1y1 = params.mx_x1y1; // xy
  float mx_x1y0 = params.mx_x1y0; // x
  float mx_x0y1 = params.mx_x0y1; // y
  float mx_x0y0 = params.mx_x0y0; // 1

  float my_x3y0 = params.my_x3y0; // xxx
  float my_x0y3 = params.my_x0y3; // yyy
  float my_x2y1 = params.my_x2y1; // xxy
  float my_x1y2 = params.my_x1y2; // yyx
  float my_x2y0 = params.my_x2y0; // xx
  float my_x0y2 = params.my_x0y2; // yy
  float my_x1y1 = params.my_x1y1; // xy
  float my_x1y0 = params.my_x1y0; // x
  float my_x0y1 = params.my_x0y1; // y
  float my_x0y0 = params.my_x0y0; // 1
  // printf("!!!!!!!! %f", fx);
  float* param_ptr;
  param_ptr = (float *) malloc(26 * sizeof(float));
  param_ptr[0] = fx;
  param_ptr[1] = fy;
  param_ptr[2] = cx;
  param_ptr[3] = cy;
  ...
  param_ptr[8] = mx_x2y1;
  param_ptr[9] = mx_x1y2;
  param_ptr[10] = mx_x2y0;
  param_ptr[11] = mx_x0y2;
  param_ptr[12] = mx_x1y1;
  param_ptr[13] = mx_x1y0;
  param_ptr[14] = mx_x0y1;
  param_ptr[15] = mx_x0y0;

  param_ptr[16] = my_x3y0;
  param_ptr[17] = my_x0y3;
  param_ptr[18] = my_x2y1;
  param_ptr[19] = my_x1y2;
  param_ptr[20] = my_x2y0;
  param_ptr[21] = my_x0y2;
  param_ptr[22] = my_x1y1;
  param_ptr[23] = my_x1y0;
  param_ptr[24] = my_x0y1;
  param_ptr[25] = my_x0y0;



  FILE* param_file = NULL;
  param_file = fopen("../params/color_param.bin", "wb");
  fwrite(param_ptr, sizeof(float), 26, param_file);
  fclose(param_file);
  free(param_ptr);
...
void Freenect2DeviceImpl::setIrCameraParams(const Freenect2Device::IrCameraParams &params)
{
  ...
  float fx = params.fx;
  float fy = params.fy; ///< Focal length y (pixel)
  float cx = params.cx; ///< Principal point x (pixel)
  float cy = params.cy; ///< Principal point y (pixel)
  float k1 = params.k1; ///< Radial distortion coefficient, 1st-order
  float k2 = params.k2; ///< Radial distortion coefficient, 2nd-order
  float k3 = params.k3; ///< Radial distortion coefficient, 3rd-order
  float p1 = params.p1; ///< Tangential distortion coefficient
  float p2 = params.p2; ///< Tangential distortion coefficient

  float* param_ptr;
  param_ptr = (float*) malloc(9 * sizeof(float));
  param_ptr[0] = fx;
  param_ptr[1] = fy;
  param_ptr[2] = cx;
  param_ptr[3] = cy;
  ...
  FILE* param_file = NULL;
  param_file = fopen("../params/ir_param.bin", "wb");
  fwrite(param_ptr, sizeof(float), 9, param_file);
  fclose(param_file);
  free(param_ptr);
